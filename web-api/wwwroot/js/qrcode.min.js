/**
 * 輕量純前端無依賴 QRCode 生成模組 (相容 QRCode.toCanvas API，100% 離線可用)
 * 基於 Kazuhiko Arase QR Code 算法之自包含極簡實現
 */
(function(window) {
    ...
    window.QRCode = {
        toCanvas: function(canvas, text, options, callback) {
            try {
                ...
                var targetWidth = options.width || 140;

                var qr = new QRCodeModel(0, 1); // Level L
                qr.addData(text);
                qr.make();

                var count = qr.getModuleCount();
                var size = count + margin * 2;
                var scale = Math.floor(targetWidth / size) || 3;
                var actualPixelSize = size * scale;

                canvas.width = actualPixelSize;
                canvas.height = actualPixelSize;
                var ctx = canvas.getContext('2d');
                ctx.fillStyle = '#ffffff';
                ctx.fillRect(0, 0, actualPixelSize, actualPixelSize);
                ...
                console.error("QRCode.toCanvas 渲染失敗:", err);
